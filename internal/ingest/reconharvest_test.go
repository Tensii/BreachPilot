...
func TestNormalizeReconSummaryPathsHandlesCopiedReportSummary(t *testing.T) {
	...
	reconRunDir := filepath.Join("artifacts", "global.com", "4", "recon", "run")
	...
	if got := filepath.ToSlash(rs.Workdir); got != "artifacts/global.com/4/recon/run" {
		t.Fatalf("expected workdir to resolve to canonical run dir, got %q", got)
	}
	if got := filepath.ToSlash(rs.Live); got != "artifacts/global.com/4/recon/run/live_hosts.txt" {
